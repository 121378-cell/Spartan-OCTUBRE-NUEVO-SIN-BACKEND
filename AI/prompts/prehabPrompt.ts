...
export const prehabSchema = {
    ...
    properties: {
        ...
        prehabRoutine: {
            type: Type.ARRAY,
            description: "Una mini-rutina de pre-habilitación para realizar antes del entrenamiento.",
            items: {
                type: Type.OBJECT,
                properties: {
                    name: { type: Type.STRING, description: "El nombre del ejercicio de pre-habilitación." },
                    instruction: { type: Type.STRING, description: "Instrucciones claras y concisas (ej: sets, reps, tempo)." }
                },
                required: ["name", "instruction"]
            }
        }
    },
    required: ["analysis", "biomechanicalAdjustments", "prehabRoutine"]
};
