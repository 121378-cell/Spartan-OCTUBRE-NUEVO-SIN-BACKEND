...
const keystoneHabitSuggestionSchema = {
    type: Type.OBJECT,
    properties: {
        name: { type: Type.STRING, description: "El nombre claro y accionable del hábito sugerido." },
        anchor: { type: Type.STRING, description: "El evento existente al que se anclará el nuevo hábito." }
    },
    required: ["name", "anchor"]
};

export const initialPlanSchema = {
    type: Type.OBJECT,
    properties: {
        routine: routineSchema,
        keystoneHabitSuggestion: keystoneHabitSuggestionSchema
    },
    required: ["routine", "keystoneHabitSuggestion"]
};