...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => { localStorage.setItem('reconditioningPlans', JSON.stringify(reconditioningPlans)); }, [reconditioningPlans]);
    ...
    useEffect(() => { localStorage.setItem('weeklyCheckIns', JSON.stringify(weeklyCheckIns)); }, [weeklyCheckIns]);
    useEffect(() => { localStorage.setItem('scheduledWorkouts', JSON.stringify(scheduledWorkouts)); }, [scheduledWorkouts]);
    ...
    const hideModal = () => setModal({ isOpen: false, type: null });
        ...
        if (response.type === 'action' && response.action) {
            ...
            switch (name) {
                ...
                case 'openModal':
                    ...
                    break;
                default:
                    console.warn(`Acción de IA desconocida: ${name}`);
            }
        }
    }, [showToast]);
